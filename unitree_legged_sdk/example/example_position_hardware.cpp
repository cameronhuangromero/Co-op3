...
void Custom::UDPRecv()
{
  ...
  udp.GetRecv(state);
    ...
    if (command_sent && !response_detected)
    {
        float actual_q = state.motorState[FR_2].q;
        float delta_q = std::abs(actual_q - baseline_q);

        if (delta_q > 0.01f)  // threshold to detect motion
        {
            response_detected = true;
            auto response_time = std::chrono::high_resolution_clock::now() - command_issue_time;

            std::cout << "[DETECTED] Hardware response to sinusoidal command detected!" << std::endl;
            std::cout << "Time to detect motion: "
                      << std::chrono::duration_cast<std::chrono::microseconds>(response_time).count()
...
void Custom::RobotControl()
{
  ...
  if (motiontime >= 0)
  {
    ...
    if (motiontime >= 400)
    {
      ...
      if (!command_sent)
    {
        baseline_q = state.motorState[FR_2].q;
        command_issue_time = std::chrono::high_resolution_clock::now();
        command_sent = true;
        response_detected = false;

        std::cout << "[INFO] Sinusoidal command issued. Baseline q = " << baseline_q << std::endl;
    }
