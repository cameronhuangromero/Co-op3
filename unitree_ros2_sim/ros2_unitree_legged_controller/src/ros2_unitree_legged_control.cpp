...
void UnitreeLeggedController::setCmdCallback(const ros2_unitree_legged_msgs::msg::MotorCmd::SharedPtr msg){
  // RCLCPP_INFO(get_node()->get_logger(), "Received command: q=%f, dq=%f, tau=%f", msg->q, msg->dq, msg->tau);
  // if (joint_name_ == "FL_calf_joint") {
  
  // RCLCPP_INFO(get_node()->get_logger(), 
  //             "Received command - Mode: %d, Q: %f, Kp: %f, Dq: %f, Kd: %f, Tau: %f",
  //             msg->mode, msg->q, msg->kp, msg->dq, msg->kd, msg->tau);
  // }
  ...
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  "~/command", rclcpp::QoS(10).reliability(rclcpp::ReliabilityPolicy::Reliable), std::bind(&UnitreeLeggedController::setCmdCallback, this, std::placeholders::_1));
  // joints_command_subscriber_ = get_node()->create_subscription<CmdType>(
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (!joint_commands)
  {
    RCLCPP_WARN(get_node()->get_logger(), "No command received in update()");
    publishState();
  ...
  calcTorque = computeTorque(currentPos, currentVel, servoCmd);
    ...
    // Log calculated torque and position
    // if (joint_name_ == "FL_calf_joint") {
    //     // Perform calculations or updates
    //     log_joint_update(joint_name_, lastCmd, currentPos, currentVel, calcTorque, servoCmd.pos);
    // }
  ...
  command_interfaces_[0].set_value(calcTorque); // Send effort to joint
      // if (joint_name_ == "FL_calf_joint") {
      //   RCLCPP_INFO(get_node()->get_logger(), "Applying torque: %f", calcTorque);
      // }
...
void UnitreeLeggedController::publishState(){
  if (rt_controller_state_publisher_ && rt_controller_state_publisher_->trylock()) {
        // RCLCPP_INFO(get_node()->get_logger(), "Publishing State - Q: %f, Dq: %f, Tau: %f", 
        //         lastState.q, lastState.dq, lastState.tau_est);
  ...
  // RCLCPP_INFO(get_node()->get_logger(), "Publishing state: q=%f, dq=%f, tau_est=%f", lastState.q, lastState.dq, lastState.tau_est);
}
...
void UnitreeLeggedController::positionLimits(double &position){
  // RCLCPP_INFO(get_node()->get_logger(), "Applying position limits: [%f, %f]", joint_->limits->lower, joint_->limits->upper);
  clamp(position, joint_->limits->lower, joint_->limits->upper);
    // RCLCPP_INFO(get_node()->get_logger(), "Position after limit: %f", position);
  // switch (joint_type_){
  //   double upper, lower;
  //   case 0:
  //   // hip
