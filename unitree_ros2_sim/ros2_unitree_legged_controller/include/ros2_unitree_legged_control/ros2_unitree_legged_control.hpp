...
void log_joint_update(const std::string &joint_name, const CmdType &cmd,
                      double currentPos, double currentVel,
                      double calcTorque, double servoCmdPos) {
    if (joint_name == "FL_calf_joint") {
        std::stringstream log_msg;
        log_msg << "Joint: " << joint_name
                << " Pos: " << currentPos
                << " Vel: " << currentVel
                << " Torque: " << calcTorque
                << " Cmd Pos: " << servoCmdPos;
        RCLCPP_INFO(get_node()->get_logger(), log_msg.str().c_str());
    }
}
