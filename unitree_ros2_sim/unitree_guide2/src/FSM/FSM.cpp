...
FSM::FSM(CtrlComponents *ctrlComp)
    ...
    _stateList.manualJoint = new State_ManualJointControl(_ctrlComp);
    _stateList.AllJoint = new State_12Joint(_ctrlComp);
    // _executor = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
    // _executor->add_node(_stateList.manualJoint->getNode());

    // _rosSpinThread = std::thread([this]() {
    // this->_executor->spin();  // explicitly use this-> for clarity
    // });
    // _rosSpinThread.detach();  // optional: keep running in background
...
void FSM::run(){
    ...
    // std::cout << "[FSM] Current userCmd: " << static_cast<int>(_ctrlComp->lowState->userCmd)
    //       << ", State: " << _currentState->_stateNameString << std::endl;
...
FSMState* FSM::getNextState(FSMStateName stateName){
    ...
    case FSMStateName::MANUAL_JOINT_CONTROL:
        return _stateList.manualJoint;
        break;

    case FSMStateName::ALL_TORQUE_JOINT:
        return _stateList.AllJoint;
        break;
