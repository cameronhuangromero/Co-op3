...
IOROS::IOROS(rclcpp::Node::SharedPtr node_ptr) : IOInterface(){
    ...
    //     // in IOROS constructor, just after initSend();
    // auto init_pub = _nm->create_publisher<ros2_unitree_legged_msgs::msg::MotorCmd>(
    //     "/FL_calf_controller/command", 1);
    // rclcpp::sleep_for(std::chrono::milliseconds(200)); // wait for subscribers

    // ros2_unitree_legged_msgs::msg::MotorCmd init_msg;
    // init_msg.mode = 5;
    // init_msg.q = -1.3;
    // init_msg.dq = 0.0;
    // init_msg.tau = 0.0;
    // init_msg.kp = 20.0;
    // init_msg.kd = 1.0;
    // init_msg.reserve = {0, 0, 0};

    // for (int i = 0; i < 10; ++i) {
    //     std::cout << "[DEBUG] Manual init publish to FL_calf: iteration " << i << std::endl;
...
void IOROS::sendCmd(const LowlevelCmd *lowCmd){
    // static bool fl_calf_initialized = false;
    ...
    for(int i(0); i < 12; ++i){
        ...
        // std::cout << "[IOROS] Publishing motor_cmd[" << i << "]: mode=" << lowCmd->motorCmd[i].mode << std::endl;

    }
    for(int m(0); m < 12; ++m){
        // if (m == 5 && fl_calf_initialized) continue;  // only skip after first send
        // if (m == 5) {
        //     if (!fl_calf_initialized) {
        //         // Send one safe, static pose command to FL_calf
        //         ros2_unitree_legged_msgs::msg::MotorCmd init_cmd;
        //         init_cmd.mode = 5;
        //         init_cmd.q = -1.3;         // ← Match rest pose from other legs
        //         init_cmd.dq = 0.0;
        //         init_cmd.tau = 0.0;
        //         init_cmd.kp = 20.0;        // Small spring to hold
        //         init_cmd.kd = 1.0;         // Some damping
        //         init_cmd.reserve = {0, 0, 0};
                
        //         if (_servo_pub[5]) {
        //             std::cout << "[DEBUG] Publishing to FL_calf" << std::endl;
        //             _servo_pub[5]->publish(init_cmd);
        //         } 
        //             else {
        //                 std::cout << "[ERROR] _servo_pub[5] is null!" << std::endl;
        //                 }
        //         _servo_pub[m]->publish(init_cmd);
        //         fl_calf_initialized = true;
        //     }
        //     continue;
        // }
...
void IOROS::initSend(){
    ...
    // std::cout << "[DEBUG] FL_calf publisher created at index 5" << std::endl;
